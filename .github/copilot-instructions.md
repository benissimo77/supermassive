...
- **NEVER de-stabilize BaseScene:** I will NEVER add high-frequency `console.log` or socket emissions to global lifecycle methods (like `resize` or `update`) in `BaseScene.ts`. These can trigger infinite loops of event cascades and crash the environment.
- **Mandatory Git Reflog Recovery:** If I perform a `git reset --hard` or a `checkout` that loses local work, I am FORBIDDEN from re-implementing logic from memory. I MUST use `git reflog` and `git show HEAD@{n}:path/to/file` to recover the exact established syntax and patterns.
