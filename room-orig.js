...
class Room {
	...
	attachHostEvents(socket) {
		...
		socket.on('host:choosegame', (game) => {
			console.log('host:choosegame:', game);
			const Game = require(`./server/games/${game}.js`);
			this.game = new Game();
			this.game.startGame();
		})
	...
	weNeedAWinner() {
		...
		// collectVotes
		// This function will send a request to all players to vote for a candidate
		// It will then wait for all players to vote before resolving the promise
		...
		const votingStart = (candidateList) => {
			...
			.then( () => {
				...
				this.#io.to(this.host.socketid).emit('server:dayvoteresult', { candidates: candidateList, votes: votes } );
				this.registerHostResponseHandler( () => {
					...
					if (candidates.length == 0) {
						...
						this.registerHostResponseHandler( () => { votingStart(newCandidateList); });
					}
					else {
						// We have a winner
