...
export default class PlayerImageOrderingQuestion extends PlayerBaseQuestion {
    ...
    protected createAnswerUI(): void {
        ...
        const questionData = this.questionData as OrderingQuestionData;
        this.items = questionData.itemsShuffled || [];

        // Dropzone labels: start/end from extra, empty for middle positions
        this.labels = this.items.map((_, i) => {
            if (i === 0) return questionData.extra?.startLabel || '';
            if (i === this.items.length - 1) return questionData.extra?.endLabel || '';
            return '';
        });

        // Create ImageButtons — look up URL via original (unshuffled) items index
        ...
        // Create dropzones with position labels
