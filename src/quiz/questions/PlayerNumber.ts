...
export default class PlayerNumberQuestion extends PlayerBaseQuestion {
    ...
    protected createAnswerUI(): void {
        console.log('PlayerNumberQuestion::createAnswerUI (NATIVE HTML BYPASS)');

        const scaleFactor = this.scene.getUIScaleFactor() || 1;
        ...
        // 'text' type with inputmode='numeric' and pattern='[0-9]*' is perfectly optimal
        // for popping a mobile numeric keypad without invoking the native annoying up/down scrollers of type='number'
        this.htmlInput.type = 'text'; 
        this.htmlInput.maxLength = 12;
        this.htmlInput.inputMode = 'decimal';
        this.htmlInput.pattern = '[0-9\\.\\-]*';
        this.htmlInput.id = 'nativeAnswerInput';
        this.htmlInput.placeholder = 'Enter number...';
        ...
        this.htmlInput.setAttribute('enterkeyhint', 'send');
    ...
    private handleSubmit(): void {
        ...
        let val = this.htmlInput.value.trim();
        console.log('PlayerNumberQuestion submitting:', val);

        // Normalize European commas to standard decimal periods for Math conversion
        val = val.replace(/,/g, '.');
