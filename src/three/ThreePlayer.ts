...
export class ThreePlayer extends PhaserPlayer {
    ...
    /**
     * Makes the entire player interactive.
     * NOTE: this assumes that the player sits in a background panel for ease of clicking
     */
    public makeInteractive(onClick: () => void): void {

        this.setCardMode(true); // Ensure we are in card mode so that the background panel exists for clicking
        ...
        this.battleSlotBg.setInteractive({
            useHandCursor: true
        })
            .on('pointerover', () => {
